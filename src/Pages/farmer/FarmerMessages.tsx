    ...
    const attachListeners = (s: ReturnType<typeof getSocket>) => {
      ...
      const onNewMessage = (msg: ChatMessage) => {
        ...
        setConversations((prev) => {
          ...
          const resolvedName = resolveNameFromMessage(namedMsg, msg.senderId) || 'Buyer';
          if (!resolveNameFromMessage(namedMsg, msg.senderId)) hydrateParticipant(msg.senderId);
          return [{ conversationId: msg.conversationId, participant: { id: msg.senderId, fullName: resolvedName, role: namedMsg.sender?.role || 'buyer' }, lastMessage: { text: msg.text, createdAt: msg.createdAt }, unreadCount: isViewingThisSender ? 0 : 1 }, ...prev];
        });
      };
      ...
      const onMessageSent = (msg: ChatMessage) => {
        ...
        setConversations((prev) => {
          ...
          const resolvedName = resolveNameFromMessage(namedMsg, msg.receiverId) || 'Buyer';
          if (!resolveNameFromMessage(namedMsg, msg.receiverId)) hydrateParticipant(msg.receiverId);
          return [{ conversationId: msg.conversationId, participant: { id: msg.receiverId, fullName: resolvedName, role: namedMsg.receiver?.role || 'buyer' }, lastMessage: { text: msg.text, createdAt: msg.createdAt }, unreadCount: 0 }, ...prev];
        });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-80 mx-4">
            ...
            <p className="text-sm text-stone-500 text-center mb-6">
              All messages with <strong>{confirmDelete.participant.fullName}</strong> will be permanently deleted for both sides.
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDeleteConversation(confirmDelete)} className="flex-1 py-2.5 rounded-xl bg-red-600 text-white text-sm font-medium hover:bg-red-700 transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="px-5 py-4 border-b border-stone-100">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-stone-400 mt-0.5">Chat with buyers</p>
              </div>
            </div>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-stone-400" />
              <input type="text" placeholder="Search buyers..." value={search} onChange={(e) => setSearch(e.target.value)}
                className="w-full pl-9 pr-3 py-2 text-sm bg-stone-50 border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent placeholder:text-stone-400" />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {convLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-40 text-center px-4">
                <Leaf className="h-8 w-8 text-stone-300 mb-2" />
                <p className="text-sm text-stone-400">No conversations yet</p>
                <p className="text-xs text-stone-300 mt-1">Buyers will appear here</p>
              </div>
            ) : (
                    ...
                    <button onClick={() => setSelectedConvAndRef(conv)} className="w-full px-4 py-3.5 flex items-start gap-3 text-left">
                      ...
                      <div className="flex-1 min-w-0 pr-8">
                        <div className="flex justify-between items-center mb-0.5">
                          <p className={`text-sm font-semibold truncate ${isActive ? 'text-green-800' : 'text-stone-800'}`}>{conv.participant.fullName}</p>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {selectedConv ? (
              ...
              <div className="flex items-center gap-3 px-6 py-4 bg-white border-b border-stone-100 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-400 mt-0.5 capitalize">
                    ...
                    {partnerTyping && <span className="text-green-600 ml-1">· typing…</span>}
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-stone-50 text-center px-8">
              ...
              <p className="text-stone-400 text-sm max-w-xs">Select a conversation from the left to start chatting with buyers</p>
            </div>
          )}
        </div>
      </div>
